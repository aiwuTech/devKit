...
func GEOHash(addr string, precision int) (hash string, lat, lng float64) {
	...
	// bd09 ==> gcj02
	point := Bd092Gcj02(lat, lng)
	lat = point.Lat()
	lng = point.Lng()

	hash, _ = Encode(lat, lng, precision)

	return
}

func GEOHashByIp(ip string, precision int) (hash string, lat, lng float64) {
	addr, err := GetAddressViaIP(ip)
	if err != nil {
		return
	}

	return GEOHash(addr.Address, precision)
}
